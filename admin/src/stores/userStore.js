...
export const useUserStore = defineStore('users', () => {
  ...
  async function loadUsers({ force = false, q = '' } = {}) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(_extractErrorMessage(payload, 'Falha ao carregar utilizadores.'))
      }

      users.value = Array.isArray(payload) ? payload.map(_toFrontendUser) : []
      hasLoadedUsers.value = true
    } catch (e) {
      error.value = e.message || 'Falha ao carregar utilizadores.'
  ...
  async function addUser(data) {
    ...
    try {
      ...
      const payload = await _parseJsonSafe(response)
      if (!response.ok) {
        throw new Error(_extractErrorMessage(payload, 'Falha ao adicionar utilizador.'))
      }

      await loadUsers({ force: true })
    } catch (e) {
      error.value = e.message || 'Falha ao adicionar utilizador.'
    } finally {
      isLoading.value = false
    }
  }
  ...
  async function updateUser(id, data) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(_extractErrorMessage(body, 'Falha ao atualizar utilizador.'))
      }
      ...
      error.value = e.message || 'Falha ao atualizar utilizador.'
    } finally {
      isLoading.value = false
    }
  }
  ...
  async function removeUser(id) {
    ...
    try {
      ...
      const payload = await _parseJsonSafe(response)
      if (!response.ok) {
        throw new Error(_extractErrorMessage(payload, 'Falha ao remover utilizador.'))
      }

      users.value = users.value.filter(u => u.id !== id)
    } catch (e) {
      error.value = e.message || 'Falha ao remover utilizador.'
    } finally {
      isLoading.value = false
    }
  }
  ...
  async function loadProfessorRequests({ force = false } = {}) {
    ...
    error.value = null
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/admin/requests`, {
        headers: {
          ..._authHeaders(),
        },
      })
      ...
      const payload = await _parseJsonSafe(response)
      if (!response.ok) {
        throw new Error(_extractErrorMessage(payload, 'Falha ao carregar pedidos de conta docente.'))
      }
      ...
      error.value = e.message || 'Falha ao carregar pedidos de conta docente.'
  ...
  async function _decideProfessorRequest(requestId, status, reviewerNote = '') {
    ...
    error.value = null
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/admin/requests/${requestId}/decision`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          ..._authHeaders(),
        },
        body: JSON.stringify({ status, admin_comment: reviewerNote || null }),
      })

      const payload = await _parseJsonSafe(response)
      if (!response.ok) {
        throw new Error(_extractErrorMessage(payload, 'Falha ao atualizar decisão do pedido.'))
      }

      await loadProfessorRequests({ force: true })
    } catch (e) {
      error.value = e.message || 'Falha ao atualizar decisão do pedido.'
    } finally {
      isLoading.value = false
    }
  }
