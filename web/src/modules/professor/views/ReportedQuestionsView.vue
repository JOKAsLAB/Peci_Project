  ...
  <div class="space-y-8">
    ...
    <template v-else>
      ...
      <div class="space-y-8" v-else>
        ...
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          <div>
            <p
              class="text-error font-bold text-sm uppercase tracking-widest mb-1"
            >
              Atenção Necessária
            </p>
            <h3 class="text-2xl sm:text-3xl font-bold">Perguntas Reportadas</h3>
            <p class="text-text-secondary mt-1">
              Exercícios reportados por 3 ou mais alunos diferentes. Reveja e
              corrija ou dispense o report.
            </p>
          </div>
          <button
            @click="reportedStore.loadReportedExercises(true)"
            :disabled="reportedStore.isLoading"
            class="self-start inline-flex items-center gap-2 px-4 py-2 bg-background border border-white/10 rounded-btn text-sm font-bold hover:border-brand transition-all disabled:opacity-50"
          >
            <i
              :class="
                reportedStore.isLoading
        ...
        <div class="grid grid-cols-3 sm:grid-cols-3 gap-3 sm:gap-6">
          <div class="bg-surface p-4 sm:p-6 rounded-card border border-white/5">
            <p class="text-text-secondary text-xs sm:text-sm">Total Reportados</p>
            <p class="text-2xl sm:text-3xl font-bold mt-2 text-error">
              {{ reportedStore.reportedExercises.length }}
            </p>
          </div>
          <div class="bg-surface p-4 sm:p-6 rounded-card border border-white/5">
            <p class="text-text-secondary text-xs sm:text-sm">Mais Reportado</p>
            <p class="text-2xl sm:text-3xl font-bold mt-2 text-warning">
              {{ maxReportCount }}
              <span class="text-xs sm:text-sm font-normal text-text-secondary"
                >rep.</span
              >
            </p>
          </div>
        ...
        <div
          ...
          class="bg-surface rounded-card border border-white/5 p-8 sm:p-16 text-center"
        >
          <div
        ...
        >
          ...
          <table class="w-full text-left">
            ...
            >
              <tr>
                <th class="px-3 py-4 font-semibold w-8 hidden sm:table-cell">#</th>
                <th class="px-3 sm:px-6 py-4 font-semibold">Exercício</th>
                <th class="px-4 py-4 font-semibold text-center hidden md:table-cell">Tipo</th>
                <th class="px-3 sm:px-4 py-4 font-semibold text-center">Disciplina</th>
                <th class="px-4 py-4 font-semibold text-center hidden md:table-cell">Tópico</th>
                <th class="px-4 py-4 font-semibold text-center hidden md:table-cell">Dificuldade</th>
                <th class="px-3 sm:px-4 py-4 font-semibold text-center">Reports</th>
                <th class="px-4 py-4 font-semibold text-center hidden lg:table-cell">
                  Último Report
                </th>
                <th class="px-2 sm:px-6 py-4 font-semibold text-right">Ações</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-white/5">
              ...
              >
                <td class="px-3 py-4 text-text-secondary text-xs hidden sm:table-cell">
                  {{ (currentPage - 1) * pageSize + idx + 1 }}
                </td>
                <td class="px-3 sm:px-6 py-4 max-w-[180px] sm:max-w-[260px]">
                  <p class="font-medium text-sm truncate" :title="ex.question">
                    {{ ex.question }}
                  </p>
                </td>
                <td class="px-4 py-4 text-center whitespace-nowrap hidden md:table-cell">
                  <span
                ...
                <td class="px-2 sm:px-4 py-4 text-center max-w-[110px] sm:max-w-[140px]">
                  <span
                    class="bg-brand/10 text-brand text-xs font-bold px-2 sm:px-3 py-1 rounded-chip truncate block"
                    :title="ex.discipline"
                  >
                    {{ ex.discipline || `UC ${ex.id_uc}` }}
                  </span>
                </td>
                <td class="px-4 py-4 text-center max-w-[160px] hidden md:table-cell">
                  <span
                ...
                <td class="px-2 sm:px-4 py-4 text-center">
                  <span
                    class="inline-flex items-center gap-1 text-xs font-bold px-2 sm:px-3 py-1 rounded-chip"
                    :class="
                      ex.report_count >= 5
                ...
                <td
                  class="px-4 py-4 text-center text-text-secondary text-xs whitespace-nowrap hidden lg:table-cell"
                >
                  {{ formatDate(ex.last_reported_at) }}
                </td>
                <td class="px-2 sm:px-6 py-4 text-right">
                  <div
                    class="flex justify-end gap-2 sm:gap-3 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity"
    ...
    <Teleport to="body">
      ...
      >
        ...
        ></div>
        <div
          class="relative bg-surface border border-white/10 rounded-card w-full max-w-lg mx-4 sm:mx-0 p-5 sm:p-8 shadow-2xl z-10 max-h-[90vh] overflow-y-auto"
        >
          ...
          <div
            v-if="detailExercise.type === 'Multiple Choice'"
          ...
          >
            ...
            >
              ...
              <i
                ...
                class="pi pi-check text-success ml-auto text-xs"
              ></i>
            </div>
          </div>
          ...
          <!-- Opções (True/False) -->
          <div
            v-else-if="detailExercise.type === 'True/False'"
          ...
          <div
            ...
            class="bg-background rounded-btn p-4 mb-6"
          >
            <p
    ...
    <Teleport to="body">
      ...
      >
        ...
        ></div>
        <div
          class="relative bg-surface border border-white/10 rounded-card w-full max-w-2xl mx-4 sm:mx-0 shadow-2xl z-10 max-h-[90vh] flex flex-col"
        >
          <div
            class="flex justify-between items-center px-4 sm:px-8 py-5 border-b border-white/5"
          >
            <div>
              <h4 class="font-bold text-lg">Editar Exercício Reportado</h4>
              <p class="text-text-secondary text-xs mt-0.5">
                Tópico: {{ editExercise.topic_name }} ·
                {{ editExercise.discipline || `UC ${editExercise.id_uc}` }}
              </p>
            </div>
            <button
              @click="closeEdit"
          ...
          <div class="overflow-y-auto p-4 sm:p-8 space-y-6">
            <!-- Pergunta -->
            <div>
              <label
            ...
            <div v-if="editForm.type === 'Multiple Choice'">
              ...
              >
                Opções
                <span class="text-brand ml-1"
                  >— clica no círculo para marcar a correta</span
                >
              </label>
              <div class="space-y-2">
                <div
            ...
            <div>
              ...
              >
                Explicação
                <span class="text-text-secondary font-normal normal-case ml-1"
                  >(opcional)</span
                >
              </label>
              <textarea
          ...
          <div class="flex justify-end gap-3 px-4 sm:px-8 py-5 border-t border-white/5">
            <button
              @click="closeEdit"
    ...
    <Teleport to="body">
      ...
      >
        ...
        ></div>
        <div
          class="relative bg-surface border border-white/10 rounded-card w-full max-w-md mx-4 sm:mx-0 p-5 sm:p-8 shadow-2xl z-10"
        >
          <div class="flex items-center gap-4 mb-4">
            <div
              class="w-12 h-12 rounded-full bg-success/10 flex items-center justify-center flex-shrink-0"
            >
            ...
            <div>
              ...
              <p class="text-text-secondary text-sm mt-0.5">
                Os {{ dismissTarget.report_count }} reports serão apagados. O
                exercício continua publicado.
              </p>
            </div>
          </div>
          <p
            class="text-sm text-text-secondary bg-background rounded-btn p-3 mb-6 truncate"
            :title="dismissTarget.question"
          >
...
onMounted(() => {
  ...
  reportedStore
    .loadReportedExercises(false, abortController.signal)
    .catch(() => {});
});
...
const maxReportCount = computed(() => {
  ...
  return Math.max(
    ...reportedStore.reportedExercises.map((e) => e.report_count),
  );
});
...
function openEdit(ex) {
  ...
  editForm.value = {
    ...
    options: ex.solution?.options?.length
      ? [...ex.solution.options]
      : ['', '', '', ''],
    correct,
...
const typeLabel = (type) =>
  ({ 'Multiple Choice': 'Escolha Múltipla', 'True/False': 'V/F' })[type] ||
  type ||
  'Escolha Múltipla';
