<template>
  <div class="space-y-8">
    ...
    <div v-if="exerciseStore.error" class="bg-error/10 border border-error/30 text-error px-4 py-3 rounded-card text-sm">
      <i class="pi pi-exclamation-triangle mr-2"></i> {{ exerciseStore.error }}
    </div>
    ...
    <div class="grid grid-cols-4 gap-6">
      ...
      <div class="bg-surface p-6 rounded-card border border-white/5">
        ...
        <p class="text-3xl font-bold mt-2 text-brand">
          {{ exerciseStore.disciplines.length }}
        </p>
      </div>
    </div>
      ...
      <div class="flex flex-wrap items-center gap-4">
        ...
        <div class="flex items-center gap-2">
          ...
          >
            ...
            <option v-for="d in exerciseStore.disciplines" :key="d" :value="d">
              {{ d }}
            </option>
          </select>
        </div>
    ...
    <div class="bg-surface rounded-card border border-white/5 overflow-hidden relative">
      <div v-if="exerciseStore.isLoading" class="absolute inset-0 bg-black/40 backdrop-blur-sm z-20 flex items-center justify-center">
      ...
      <table class="w-full text-left">
        ...
        <tbody class="divide-y divide-white/5">
          ...
          >
            <td class="px-6 py-4 text-text-secondary text-xs">{{ ex.id }}</td>
            ...
            <td class="px-6 py-4 text-center">
              <div
                ...
                :class="{'opacity-50 pointer-events-none': exerciseStore.isLoading}"
              >
                <div
            ...
            <td class="px-6 py-4 text-right">
              ...
              >
                ...
                >
                  <i
                    :class="ex.published ? 'pi pi-eye-slash' : 'pi pi-check'"
                  ></i>
                </button>
                <button
          ...
          <div class="space-y-2 mb-4">
            ...
            >
              <span
                ...
                :class="
                  i === detailExercise.correct ? 'bg-success/20' : 'bg-white/5'
                "
...
const filteredExercises = computed(() => {
  ...
  if (disciplineFilter.value)
    list = list.filter((e) => e.discipline === disciplineFilter.value);
...
async function removeExercise(id) {
  if(exerciseStore.isLoading) return;
  if(!confirm('Tem a certeza de que pretende eliminar este exercício?')) return;
