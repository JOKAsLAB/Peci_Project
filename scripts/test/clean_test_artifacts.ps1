...
foreach ($relativeTarget in $fixedTargets) {
    $absoluteTarget = Join-Path $projectRoot $relativeTarget
...
$scopedRoots = @(
    ...
    (Join-Path $projectRoot "admin_docente")
)

if ($IncludeAiArtifacts) {
    $scopedRoots += (Join-Path $projectRoot "ai_engine")
}

if ($IncludeAlunoArtifacts) {
    $scopedRoots += (Join-Path $projectRoot "aluno")
    $alunoTargets = @(
        "aluno/build",
        "aluno/build_runtime",
        "aluno/.dart_tool"
    ...
    foreach ($relativeTarget in $alunoTargets) {
        $absoluteTarget = Join-Path $projectRoot $relativeTarget
        Remove-TargetIfExists -AbsolutePath $absoluteTarget -RemovedCount ([ref]$removed) -SkippedCount ([ref]$skipped)
    }
}
...
if (-not $KeepTempLogs) {
    $tempLogs = @(
        ...
        (Join-Path $env:TEMP "peci_admin_docente_backend.out.log"),
        (Join-Path $env:TEMP "peci_admin_docente_backend.err.log")
    )
