...
try {
    if ($PubGet) {
        Write-Host "[TEST][ALUNO] A executar flutter pub get..." -ForegroundColor Cyan
        flutter pub get
    ...
    Write-Host "[TEST][ALUNO] A executar widget tests..." -ForegroundColor Cyan
    flutter test test/widget_test.dart
    if ($LASTEXITCODE -ne 0) {
        throw "Falha nos widget tests (codigo $LASTEXITCODE)."
    }

    Write-Host "[TEST][ALUNO] Widget tests concluidos com sucesso." -ForegroundColor Green
}
finally {
    Pop-Location
}
