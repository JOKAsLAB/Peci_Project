...
function _makeAcronym(name, fallback) {
  ...
  const normalized = String(name || '')
    ...
    .map((word) =>
      romanNumerals.has(word.toUpperCase())
        ? word.toUpperCase()
        : word[0].toUpperCase(),
    )
    .join('');
...
export const useDisciplineStore = defineStore('disciplines', () => {
  ...
  const activeDisciplines = computed(() =>
    disciplines.value.filter((d) => d.active),
  );
  const inactiveDisciplines = computed(() =>
    disciplines.value.filter((d) => !d.active),
  );
  const totalStudents = computed(() =>
    disciplines.value.reduce((sum, d) => sum + d.students, 0),
  );
  ...
  async function loadDisciplines({ force = false } = {}) {
    ...
    try {
      ...
      disciplines.value = Array.isArray(payload)
        ? payload.map(_toFrontendDiscipline)
        : [];
      hasLoaded.value = true;
    } catch (e) {
      error.value = getApiErrorMessage(
        e,
        'Falha ao carregar unidades curriculares.',
      );
  ...
  async function addDiscipline(data) {
    ...
    try {
      const payload = {
        ...
        professor_ids: Array.isArray(data.professorIds)
          ? data.professorIds
          : [],
      };
      ...
      error.value = getApiErrorMessage(
        err,
        'Falha na comunicação com o servidor ao criar disciplina.',
      );
    } finally {
      isLoading.value = false;
    }
  ...
  async function updateDiscipline(id, data) {
    ...
    try {
      ...
      error.value = getApiErrorMessage(
        err,
        'Falha ao atualizar dados da disciplina.',
      );
    } finally {
      isLoading.value = false;
    }
  ...
  async function toggleStatus(id) {
    error.value =
      'O estado ativo/inativo de UCs ainda não está exposto no backend.';
  }
