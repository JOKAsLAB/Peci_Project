...
export const useAuthStore = defineStore('auth', () => {
  ...
  function setOfflineSession(email: string, role: SupportedRole, remember = true): AuthUser {
    ...
    setSession({ access_token: 'local-dev-token', user: offlineUser }, remember)
    return offlineUser
  }

  async function login(email: string, password: string, options: LoginOptions): Promise<AuthUser> {
    const remember = options.remember !== false
    const allowOfflineFallback = options.allowOfflineFallback !== false

    try {
      const { data: loginData } = await http.post<AuthPayload>('/api/v1/auth/login', {
        email,
        password,
      })
      ...
      if (loginData.user?.role !== options.role) {
        throw new Error(`Esta conta não tem permissões de ${options.role === 'Admin' ? 'administrador' : 'docente'}.`)
      }

      setSession(loginData, remember)
      return loginData.user
    } catch (error) {
      if (allowOfflineFallback && axios.isAxiosError(error) && !error.response) {
  ...
  async function registerProfessor(payload: {
    ...
    try {
      ...
      throw new Error(getApiErrorMessage(error, 'Falha ao registar conta docente.'))
    }
  }
